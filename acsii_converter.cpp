...
void ACSII_Converter::ConvertAndOutput(std::string path, int WINDOW_SIZE_X, int WINDOW_SIZE_Y, int RESOLUTION_X, int RESOLUTION_Y, int SYMPHOL_WIDTH, int SYMPHOL_HEIGHT)
{
    ...
    for(int py = 0; py < RESOLUTION_Y; py++)
    {
        ...
        {
            gs=0;
            for(int y = py * SYMPHOL_HEIGHT; y < py * SYMPHOL_HEIGHT+SYMPHOL_HEIGHT; y+=1)
            {
                for(int x = px * SYMPHOL_WIDTH; x < px * SYMPHOL_WIDTH + SYMPHOL_WIDTH; x+=1)
                {
...
void ACSII_Converter::ConvertAndPutToStream(QImage im, int WINDOW_SIZE_X, int WINDOW_SIZE_Y, int RESOLUTION_X, int RESOLUTION_Y, int SYMPHOL_WIDTH, int SYMPHOL_HEIGHT, QTextStream *stream)
{
    ...
    for(int py = 0; py < RESOLUTION_Y; py++)
    {
        for(int px = 0; px < RESOLUTION_X; px++)
        {
            ...
            *stream << grayness.toStdString()[gs / (SYMPHOL_WIDTH * SYMPHOL_HEIGHT) * grayness.size() / 256];
        }
        *stream << '\n';
    }
